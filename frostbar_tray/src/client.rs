...
use zbus::{
    ...
    names::InterfaceName,
    zvariant::{Array, Structure, Value},
};
...
use crate::{
    data::TrayItemMap,
    error::{Error, Result},
    item::{self, IconPixmap, Status, StatusNotifierItem, Tooltip},
    menu::{MenuDiff, TrayMenu},
    names,
    proxy::{
        DBusMenuProxy, DBusProps, OwnedValueExt, PropertiesUpdate,
        StatusNotifierItemProxy, StatusNotifierWatcherProxy,
    },
...
impl Client {
    ...
    pub async fn new() -> Result<Self> {
        ...
        let wellknown = loop {
            use zbus::fdo::RequestNameReply::{
                AlreadyOwner, Exists, InQueue, PrimaryOwner,
            };

            i += 1;
            let wellknown =
                format!("org.freedesktop.StatusNotifierHost-{pid}-{i}");
        ...
        // otherwise some incoming items get missed
        {
            ...
            spawn(async move {
                let initial_items =
                    watcher_proxy.registered_status_notifier_items().await?;
    ...
    async fn handle_item(
        ...
        let properties =
            Self::get_item_properties(destination, &path, &properties_proxy)
                .await?;
        ...
        if let Some(menu) = properties.menu {
            ...
            spawn(async move {
                Self::watch_menu(destination, &menu, &connection, tx, items)
                    .await?;
                Ok::<(), Error>(())
            });
        }
    ...
    async fn get_item_properties(
        ...
        let properties = match properties {
            ...
            Err(err) => {
                error!(
                    "Error fetching properties from {destination}{path}: {err:?}"
                );
                return Err(err.into());
            }
        };
    ...
    async fn watch_item_properties(
        ...
        let mut disconnect_stream =
            dbus_proxy.receive_name_owner_changed().await?;
        let mut props_changed =
    ...
    async fn get_update_event(
        ...
        use UpdateEvent::{
            AttentionIcon, Icon, OverlayIcon, Status, Title, Tooltip,
        };

        let header = change.header();
        let member = header.member().ok_or(Error::InvalidData(
            "Update message header missing `member`",
        ))?;
    ...
    async fn watch_menu(
        ...
        let menu = TrayMenu::try_from(menu.1)?;

        items.update_menu(&destination, &menu);

        tx.send(Event::Update(destination.clone(), UpdateEvent::Menu(menu)))?;

        let mut layout_updated =
            dbus_menu_proxy.receive_layout_updated().await?;
        let mut properties_updated =
