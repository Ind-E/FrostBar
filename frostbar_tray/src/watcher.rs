...
impl StatusNotifierWatcher {
    ...
    async fn register_status_notifier_host(
        ...
        let added_first = {
            ...
            let mut hosts =
                self.hosts.lock().expect("mutex lock should succeed");
        ...
        StatusNotifierWatcher::status_notifier_host_registered(&signal_emitter)
            .await?;
    ...
    async fn register_status_notifier_item(
        ...
        {
            let mut items =
                self.items.lock().expect("mutex lock should succeed");
        ...
        self.registered_status_notifier_items_changed(&signal_emitter)
            .await?;
        StatusNotifierWatcher::status_notifier_item_registered(
            &signal_emitter,
            item.as_ref(),
        )
        .await?;
    ...
    async fn unregister_status_notifier_item(
        ...
        if let Err(err) =
            Self::status_notifier_item_unregistered(&context, &item).await
        {
            error!("{err:?}");
        }
