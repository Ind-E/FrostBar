...
use crate::proxy::{MenuLayout, PropertiesUpdate, UpdatedProps};
...
#[derive(
    Debug, Deserialize, Serialize, Copy, Clone, Eq, PartialEq, Default,
)]
pub enum MenuType {
    ///  a separator
    Separator,
...
impl TryFrom<&OwnedValue> for MenuItem {
    ...
    fn try_from(value: &OwnedValue) -> Result<Self> {
        ...
        if let Some(Value::Dict(dict)) = fields.next() {
            ...
            menu.icon_name =
                dict.get::<&str, &str>(&"icon-name")?.map(str::to_string);
...
impl TryFrom<PropertiesUpdate<'_>> for Vec<MenuDiff> {
    ...
    fn try_from(value: PropertiesUpdate<'_>) -> Result<Self> {
        ...
        for removed in value.removed {
            ...
            update.remove =
                removed.fields.iter().map(ToString::to_string).collect();
        }
...
impl TryFrom<UpdatedProps<'_>> for MenuItemUpdate {
    ...
    fn try_from(value: UpdatedProps) -> Result<Self> {
        ...
        Ok(Self {
            label: dict.get("label").map(|v| {
                v.downcast_ref::<&str>().map(ToString::to_string).ok()
            }),
            ...
            icon_name: dict.get("icon-name").map(|v| {
                v.downcast_ref::<&str>().map(ToString::to_string).ok()
            }),
