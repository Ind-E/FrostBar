use crate::modules::spectrum::pipewire::util::pipewire::dict::dict_to_map;
use pipewire as pw;
use pw::{registry::GlobalObject, spa::utils::dict::DictRef};
...
impl GraphNode {
    ...
    pub fn matches_name(&self, candidate: &str) -> bool {
        self.name.as_deref() == Some(candidate)
            || self.description.as_deref() == Some(candidate)
    }
...
pub fn pair_ports_by_channel(
    ...
    let mut targets_by_channel: HashMap<String, VecDeque<GraphPort>> =
        HashMap::new();
    ...
    for source in sources {
        ...
        if let Some(channel) = source.channel_key()
            && let Some(queue) =
                targets_by_channel.get_mut(&normalize_channel_name(channel))
        {
