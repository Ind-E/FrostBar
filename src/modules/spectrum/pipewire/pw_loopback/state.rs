use super::{DeviceCaptureTarget, LoopbackMode, SINK_NAME};
use crate::modules::spectrum::pipewire::util::pipewire::{
    DefaultTarget, GraphNode, GraphPort, PortDirection,
    create_passive_audio_link, pair_ports_by_channel, parse_metadata_name,
};
use pipewire as pw;
...
impl LoopbackState {
    ...
    pub(super) fn upsert_node(&mut self, node: GraphNode) {
        ...
        if entry.has_name(SINK_NAME) {
            self.set_node(Some(node_id));
        } else if self.node_id == Some(node_id) {
            self.set_node(None);
        }
    ...
    pub(super) fn remove_port_by_global(&mut self, global_id: u32) -> bool {
        let Some((node_id, port_id)) = self.port_index.remove(&global_id)
        else {
            return false;
        };
    ...
    pub(super) fn update_default_sink(
        ...
        let changed = self.default_sink.update(
            metadata_id,
            subject,
            type_hint,
        ...
        if changed {
            let message = match (
                self.default_sink.node_id,
                self.default_sink.name.as_deref(),
            ) {
                (Some(node_id), _) => {
                    format!("default audio sink is node #{node_id}")
                }
                (None, Some(name)) => {
                    format!(
                        "default audio sink set to '{name}' (node unresolved)"
                    )
                }
    ...
    pub(super) fn set_mode(&mut self, mode: LoopbackMode) {
        ...
        match mode {
            LoopbackMode::ForwardToDefaultSink => {
                info!(
                    "[loopback] forwarding FrostBar sink to default hardware sink"
                );
            }
            LoopbackMode::CaptureFromDevice(DeviceCaptureTarget::Default) => {
                info!(
                    "[loopback] capturing audio from default hardware sink monitor"
                );
            }
            LoopbackMode::CaptureFromDevice(DeviceCaptureTarget::Node(id)) => {
                if let Some(node) =
                    self.nodes.get(&id).and_then(|node| node.info.as_ref())
                {
                    let label = node
                        .description()
                        .or_else(|| node.name())
                        .unwrap_or("unnamed");
                    info!(
                        "[loopback] capturing audio from node #{} ({label})",
                        id
                    );
                } else {
                    info!(
                        "[loopback] capturing audio from node #{id} (details unresolved)"
                    );
                }
            }
        }
    ...
    fn refresh_links(&mut self) {
        let plan = match self.mode {
            ...
            LoopbackMode::CaptureFromDevice(target) => {
                self.compute_capture_plan(target)
            }
        };
    ...
    fn compute_capture_plan(
        &mut self,
        capture_target: DeviceCaptureTarget,
    ) -> Option<RoutePlan> {
        let target_id = self.node_id?;
            ...
            DeviceCaptureTarget::Default => {
                ...
                self.default_sink.node_id.or_else(|| {
                    ...
                    self.nodes
                        .iter()
                        .find_map(|(&id, node)| {
                            node.matches_name(name).then_some(id)
                        })
                        .inspect(|&id| self.default_sink.node_id = Some(id))
                })?
            }
            DeviceCaptureTarget::Node(id) => {
                self.nodes.contains_key(&id).then_some(id)?
            }
        };

        (source_id != target_id)
            .then(|| self.build_route_plan(source_id, target_id))?
    }
    ...
    fn build_route_plan(
        ...
        let source_ports =
            self.select_ports(source_id, "source", "output", |node| {
                node.output_ports_for_loopback()
            })?;
        let target_ports =
            self.select_ports(target_id, "target", "input", |node| {
    ...
    fn set_node(&mut self, node_id: Option<u32>) {
        if self.node_id != node_id {
            match node_id {
                Some(id) => {
                    info!("[loopback] detected FrostBar sink node #{id}")
                }
                None if self.node_id.is_some() => {
                    info!("[loopback] FrostBar sink node removed")
                }
                None => {}
            }
