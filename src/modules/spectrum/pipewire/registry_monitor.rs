use super::{
    config::{CaptureMode, DeviceSelection, RoutingCommand},
    util::log,
};
use crate::modules::spectrum::pipewire::{
    VIRTUAL_SINK_NAME, pw_loopback, pw_registry, pw_router,
};
...
pub fn init_registry_monitor(
    ...
    std::thread::Builder::new()
        .name("frostbar-registry-monitor".into())
        .spawn(move || {
            run_monitor_loop(handle_for_thread, command_rx, snapshot_tx)
        })
        .inspect_err(|err| {
        ...
        self.target != desired
            ...
            || (self.failures > 0
                && self.last_attempt.is_none_or(|last| {
                    now.duration_since(last) >= ROUTE_RETRY_INTERVAL
                }))
    }
}
...
impl RoutingManager {
    ...
    fn apply_snapshot(
        ...
        let Some(sink) = snapshot.find_node_by_label(VIRTUAL_SINK_NAME) else {
            if log_sink_missing && !self.sink_missing_warned {
                warn!(
                    "[router] virtual sink '{VIRTUAL_SINK_NAME}' not yet available"
                );
    ...
    fn route_to_target(
        ...
        let now = Instant::now();
        let should_route =
            self.routed_nodes.get(&node.id).is_none_or(|state| {
                state.should_retry(desired, now, self.router_epoch)
            });
    ...
    fn ensure_router(&mut self) -> Option<&pw_router::Router> {
        ...
        if self.router.is_some() {
            warn!(
                "[router] detected stale metadata proxy; scheduling reconnection"
            );
            self.router = None;
            self.router_retry_after =
                Some(Instant::now() + ROUTER_RECOVERY_DELAY);
        }
        ...
        match (self.capture_mode, self.device_target) {
            ...
            (CaptureMode::Device, DeviceSelection::Node(id)) => {
                if snapshot.nodes.iter().any(|n| n.id == id) {
                    self.device_missing_warned = false;
                    LoopbackMode::CaptureFromDevice(DeviceCaptureTarget::Node(
                        id,
                    ))
                } else {
                    if !self.device_missing_warned {
                        warn!(
                            "[router] capture node #{id} unavailable; using default"
                        );
                        self.device_missing_warned = true;
                    }
                    LoopbackMode::CaptureFromDevice(
            ...
            let (target, desired) = match self.capture_mode {
                CaptureMode::Applications => {
                    if self.is_node_enabled(node.id) {
                        ...
                        match hardware_sink {
                            Some(hw) => {
                                (Some(hw), RouteTarget::Hardware(hw.id))
                            }
